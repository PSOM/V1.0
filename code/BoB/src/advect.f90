...
!Sonaljit -- This part is commented out, and flux limiting algorithm is applied
...
!Sonaljit - This is the part that does the flux limiting algorithm
               absvel= abs(uf(i,j,k))
               upos= Half*(uf(i,j,k) +absvel)
               uneg= Half*(uf(i,j,k) -absvel)

               left= Eighth*(dvarx(i) -dvarx(i-1))
               right= Eighth*(dvarx(i+1) -dvarx(i))
               ctr= Half* ( var(i+1,j,k ) + var(i,j,k ))
               varflx(i,j,k) = uf(i,j,k)*ctr - upos*left -uneg*right
!! -----------------------------------------------------------------
...
! -- This is the flux limiter code along Y. But it is not implemented as of now.
!      do j=1:NJ-1

!               absvel= abs(vf(i,j,k))
!               vpos= Half*(vf(i,j,k) +absvel)
!               vneg= Half*(vf(i,j,k) -absvel)

!               left= Eighth*(dvary(i) -dvary(i-1))
!               right= Eighth*(dvary(i+1) -dvary(i))
!               ctr= Half* ( var(i+1,j,k ) + var(i,j,k ))
!               varflx(i,j,k) = vf(i,j,k)*ctr - vpos*left -vneg*right
