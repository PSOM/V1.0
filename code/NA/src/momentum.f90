...
!  use relaxation
  ...
  CALL advection_and_mixing(0,1,dtim,step) 
  ...
  CALL rpevalgrad_Song(1)   ! calculates baroclinic press gradients
  !      call rpflatbtm(1)                                                
  !      call viscous(0)                                                  
  CALL coriolis(0) 
  ...
  CALL sigma 
  !     advecn(m,n,dtim) means use level m and write s,T to level n       
  CALL advection_and_mixing(1,1,dtim,step) 
  !     call intpol with the time level at which uf is known              
  !     besides interpolating the convective terms, intpol computes u*    
  CALL intpol 
  !     we call the following 3 after advecn, in which case               
  ...
  CALL sigma 
  !     advecn(m,n,dtim) means use level m and write s,T to level n       
  CALL advection_and_mixing(1,0,dtim,step) 
  !     call intpol with the time level at which uf is known              
  !     besides interpolating the convective terms, intpol computes u*    
  CALL intpol 
