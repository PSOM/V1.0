...
INTEGER OMP_GET_THREAD_NUM,OMP_GET_NUM_THREADS,CHUNK,NPROC
  ...
  !$OMP PARALLEL SHARED(var,uf) PRIVATE(k,j,dvarx,left,ctr,fc,i2,right)
  
  !$OMP DO
  do k=1,NK 
    do j=1,NJ 
      !PRINT*," OpenMP:",k,"   ",j,"    ,N_threads = ",OMP_GET_NUM_THREADS()," thread = ", OMP_GET_THREAD_NUM() 
  ...
  !$OMP PARALLEL SHARED(var,vf) PRIVATE(k,i,dvary,left,ctr,fc,right)
  ...
  !$OMP PARALLEL SHARED(var,wf) PRIVATE(j,i,dvarz,left,ctr,fc,right) 
