...
INTEGER OMP_GET_THREAD_NUM,OMP_GET_NUM_THREADS,CHUNK,NPROC
  ...
  !$OMP PARALLEL DO PRIVATE(k,j,dvarx,left,ctr,fc,i2,right) SHARED(var,uf)
  do k=1,NK 
    do j=1,NJ 
!      write(6,"(a,i3,a,i3,a,i2,a,i2)") " OpenMP:",k,"   ",j,"    ,N_threads = ",OMP_GET_NUM_THREADS()," thread = ", OMP_GET_THREAD_NUM() 
  ...
  !$OMP PARALLEL DO PRIVATE(k,i,dvary,left,ctr,fc,right) SHARED(var,vf)                                                               
  do k=1,NK 
    do i=1,NI
  ...
  !$OMP PARALLEL DO PRIVATE(j,i,dvarz,left,ctr,fc,right) SHARED(var,wf)
  do j=1,NJ 
    do i=1,NI 
