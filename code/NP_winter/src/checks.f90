...
#include "cppdefs.h"
    ...
    if(verbose) then
        ! Open a file for model configuration
        open(24, file=TRIM(dirout)//'model_configuration_'//trim(str)//'.out', status="new", action="write")
        ...
        write(24,*) "#----------------  MODEL SETTING     "
        write(6,*) "#                                           "
        write(24,*) "#                                           "


        write(6,*)             "#   time step   x number of time steps  = length of simulation "
        write(24,*)             "#   time step   x number of time steps  = length of simulation "
        ...
        if(r1>24) then
            ...
            write(24,"(A3,F8.2,A13,I6,A12,I5,A10,F7.3,A2)") " # ",dtime_dim," sec  x      ",nsteps,"           = ",n1," days and ",r1," h"
        else
            write(6,"(A3,F8.2,A13,I6,A12,F7.3,A2)") " # ",dtime_dim," sec  x      ",nsteps,"           = ",nsteps*dtf*TL/3600., "h"
            write(24,"(A3,F8.2,A13,I6,A12,F7.3,A2)") " # ",dtime_dim," sec  x      ",nsteps,"           = ",nsteps*dtf*TL/3600., "h"
        endif
        ...
        write(24,*) "# only rho is used."
#else
        write(6,*) "# s and T are used."
        write(24,*) "# s and T are used."
#endif
        ...
        write(24,"(A47,2(D11.3))") "# horizontal diffusion coefficients (m2.s-1) :",Kx,Ky

#ifdef fixed_bottom_thickness
        IF(bottom_linear_drag) then
            write(6,"(A46,D11.3)") " # bottom drag is linear, with coefficient RR=",RR
            write(24,"(A46,D11.3)") " # bottom drag is linear, with coefficient RR=",RR
        ELSE
        ...
        write(24,*) "#----------------  NONDIMENSIONAL NUMBERS    "
        write(6,*) "#                                            "
        write(24,*) "#                                            "
        ...
        WRITE(24,"(A,E14.4)")  " # EPS     = ",EPS
        WRITE(24,"(A,E14.4)")  " # delta   = ",delta
        WRITE(24,"(A,E14.4)")  " # qpr     = ", qpr 
        WRITE(24,"(A,E14.4)")  " # lambda  = ",lambda
        WRITE(24,"(A,E14.4)")  " # beta    = ", beta
        WRITE(24,"(A,E14.4)")  " # dtf     = ", dtf 
        WRITE(24,"(A,E14.4)")  " # WL      = ", WL
        WRITE(24,"(A,E14.4)")  " # R0      = ", R0
        WRITE(24,"(A,E14.4)")  " # UL      = ", UL
        WRITE(24,"(A,E14.4)")  " # P1      = ", P1
        WRITE(24,"(A,E14.4)")  " # HL      = ", HL
        WRITE(24,"(A,E14.4)")  " # HDL     = ", HDL   

        write(6,*) "#                                           "
        write(24,*) "#                                           "
        write(6,*) "#----------------  GRID CHARACTERISTICS     "
        write(24,*) "#----------------  GRID CHARACTERISTICS     "
        write(6,*) "#                                           "
        write(24,*) "#                                           "
        if(rect) then
            write(6,*) "# rectangular grid" 
            write(24,*) "# rectangular grid" 
        else 
            write(6,*) "# non-rectangular grid !"
            write(24,*) "# non-rectangular grid !"
        endif
        if(periodicew) then
            write(6,*) "# periodic in the EW direction" 
            write(24,*) "# periodic in the EW direction" 
        else 
            write(6,*) "# non periodic in the EW direction (not supported!)"
            write(24,*) "# non periodic in the EW direction (not supported!)"
        endif


        write(6,*) "# NI, NJ, NK: ",NI,NJ,NK
        write(24,*) "# NI, NJ, NK: ",NI,NJ,NK
        write(6,"(A12,F8.2,A2)") " # delta_x: ",dx, " m"
        write(24,"(A12,F8.2,A2)") " # delta_x: ",dx, " m"
        write(6,"(A12,F8.2,A2)") " # delta_y: ",dy, " m"
        write(24,"(A12,F8.2,A2)") " # delta_y: ",dy, " m"
        ...
        write(24,"(A33,F8.4)") " # top cell thickness (m)      : ",dztop
   
#ifdef fixed_bottom_thickness
        write(6,"(A33,F8.4)") " # bottom cell thickness (m)   : ",dzbot
        write(24,"(A33,F8.4)") " # bottom cell thickness (m)   : ",dzbot
#else
        write(6,*) "# bottom cell height       : variable"
        write(24,*) "# bottom cell height       : variable"
#endif

        write(6,"(A39,F7.3)") " # vertical grid stretching factor   : ",pfac
        write(24,"(A39,F7.3)") " # vertical grid stretching factor   : ",pfac
 
        if(lv_flat_bottom) then
            write(6,"(A27,F8.3,A2)") " # flat topography, depth: ",total_depth," m"
            write(24,"(A27,F8.3,A2)") " # flat topography, depth: ",total_depth," m"
        else
            write(6,"(A40,F9.3,A7,F9.3,A2)") " # sloping topography, varying between: ",&
                                       &MINVAL(zf(:,:,0)*DL)," m and ",MAXVAL(zf(:,:,0)*DL)," m"
            write(24,"(A40,F9.3,A7,F9.3,A2)") " # sloping topography, varying between: ",&
                                       &MINVAL(zf(:,:,0)*DL)," m and ",MAXVAL(zf(:,:,0)*DL)," m"                                       
        endif
        ...
        write(24,*) "#-----------  PARTICLES CHARACTERISTICS     "
        write(6,*) "#                                           "
        write(24,*) "#                                           "
        ...
        write(6,*) "# frequency of seeding  : ",parti_inifreq
        ...
        write(24,*) "# particles ENABLED."
        write(24,*) "# number of particles   : ",NPR
        write(24,*) "# initial time step     : ",ini_particle_time
        write(24,*) "# frequency of seeding  : ",parti_inifreq
        write(24,*) "# frequency of output   : ",parti_outfreq
        write(24,*) "# number of output files: ",parti_file_num
        write(24,"(A22,4(E14.7,' , '))") " # pcx, pcy, pcz, pcr: ",pcx,pcy,pcz,pcr
#else
        write(6,*) "# particles DISABLED. "
        write(24,*) "# particles DISABLED. "
#endif 
        ...
        write(24,*) "#----------------  OUTPUT CHARACTERISTICS     "
        write(6,*) "#                                           "
        write(24,*) "#                                           "
        ...
        write(24,"(A22,A101)") " # output ENABLED, in  ",dirout


#ifdef file_output_cdf
        write(6,*) "# output in cdf format ENABLED."
        write(24,*) "# output in cdf format ENABLED."
#else  
        write(6,*) "# output in cdf format DISABLED."
        write(24,*) "# output in cdf format DISABLED."
#endif

#ifdef file_output_bin
        write(6,*) "# output in bin format ENABLED."
        write(24,*) "# output in bin format ENABLED."
#else  
        write(6,*) "# output in bin format DISABLED."
        write(24,*) "# output in bin format DISABLED."
#endif


        write(6,*) "# output 1D frequency: ",out1d_int,"steps"
        write(24,*) "# output 1D frequency: ",out1d_int,"steps"
        write(6,*) "# output 2D frequency: ",out2d_int,"steps"
        write(24,*) "# output 2D frequency: ",out2d_int,"steps"
        write(6,*) "# output 3D frequency: ",out3d_int,"steps"
        write(24,*) "# output 3D frequency: ",out3d_int,"steps"


#else  

        write(6,*) "# output DISABLED."
        write(24,*) "# output DISABLED."
        ...
        write(24,*) "#                                           "
        write(24,*) "#----------------  USER VARIABLES           "
        write(24,*) "#                                           "
        write(24,*) "# user1 : ",user1
        write(24,*) "# user2 : ",user2
        write(24,*) "# user3 : ",user3
        write(24,*) "# user4 : ",user4
        write(24,*) "# user5 : ",user5
        write(24,*) "# user6 : ",user6
        write(24,*) "# user7 : ",user7
        write(24,*) "# user8 : ",user8
        ...
        write(24,*) "############################################"
        write(24,*) "#                                           "
        write(24,*) "#----------------  INITIAL SETTING     "
        write(24,*) "#                                           "
        write(24,"(A21,D15.7,A5,D15.7)") " #   u lies between: ",MINVAL(u(:,:,:,0))," and",MAXVAL(u(:,:,:,0))    
        write(24,"(A21,D15.7,A5,D15.7)") " #   v lies between: ",MINVAL(v(:,:,:,0))," and",MAXVAL(v(:,:,:,0))    
        write(24,"(A21,D15.7,A5,D15.7)") " #   w lies between: ",MINVAL(w(:,:,:,0))," and",MAXVAL(w(:,:,:,0))    

#ifdef rhoonly
        write(6,"(A21,D15.7,A5,D15.7,A11)") " # rho lies between: ",MINVAL(rho(:,:,:))," and",MAXVAL(rho(:,:,:))," (only rho)"    
        write(24,"(A21,D15.7,A5,D15.7,A11)") " # rho lies between: ",MINVAL(rho(:,:,:))," and",MAXVAL(rho(:,:,:))," (only rho)"    

#else
        write(6,"(A21,D15.7,A5,D15.7)") " #   s lies between: ",MINVAL(s(:,:,:,0))," and",MAXVAL(s(:,:,:,0))    
        write(24,"(A21,D15.7,A5,D15.7)") " #   s lies between: ",MINVAL(s(:,:,:,0))," and",MAXVAL(s(:,:,:,0))    
        write(6,"(A21,D15.7,A5,D15.7)") " #   T lies between: ",MINVAL(T(:,:,:,0))," and",MAXVAL(T(:,:,:,0))    
        write(24,"(A21,D15.7,A5,D15.7)") " #   T lies between: ",MINVAL(T(:,:,:,0))," and",MAXVAL(T(:,:,:,0))    
        write(6,"(A21,D15.7,A5,D15.7,A19)") " # rho lies between: ",MINVAL(rho(:,:,:))," and",MAXVAL(rho(:,:,:))," (rho from s and T)"
        write(24,"(A21,D15.7,A5,D15.7,A19)") " # rho lies between: ",MINVAL(rho(:,:,:))," and",MAXVAL(rho(:,:,:))," (rho from s and T)"
#endif
        write(6,"(A21,D15.7,A5,D15.7)") " #   h lies between: ",MINVAL(h(:,:))," and",MAXVAL(h(:,:))    
        write(24,"(A21,D15.7,A5,D15.7)") " #   h lies between: ",MINVAL(h(:,:))," and",MAXVAL(h(:,:))    
        write(6,*) "#                                           "
        write(24,*) "#                                           "
        write(6,*) "############################################"
        write(24,*) "############################################"
