...
#include "size.h"
 ...
 !--------------------------
  ...
  LOGICAL, PARAMETER :: bottom_linear_drag=.TRUE.            !@ only linear drag is currently supported (see in 
                                                             !  mixing_vertical routine). 
 ...
 !---------------------
  ...
  REAL(kind=rc_kind) ::  delta,delinv                        !@ delta: ratio of horizontal to vertical length scale = D/L 
 ...
 !------------------------                                               
  ...
  integer :: parti_outfreq, parti_inifreq                                        
...
!------------------------------------------------------------------------!
  ...
  REAL(kind=rc_kind), dimension(    0:NI+1,0:NJ+1, 0:NK+1)      :: zc,wx,wy,wz,p,strain,shear,Jac,Jacinv,rho,rho_bar,rho_pri,vor,&
                                                                    &pv,divQ
...
REAL(kind=rc_kind) :: sasouth(NK),Tsouth(NK),snorth(NK),Tnorth(NK)
