...
! Main loop:
 ...
 time_seconds= time_nondim*TL
  ...
  if (step == ini_particle_time) then
    PRINT*,"INITIALIZATION OF ",NPR ," PARTICLES "
    CALL ini_particles(step)
    CALL get_parti_vel(step)    ! Get the particle velocity for the first step
    CALL save_parti             ! Save the release position of the particles
    ! Apply a Euler forward scheme at the first time step
    DO i = 1, NP
  ...
  CALL diag_energy(step)                   !     compute the TKE                                                    
  CALL meanh(NI,NJ,h,hmean) 

#ifdef allow_particle
  if (step>ini_particle_time-1) then
    if (step>ini_particle_time .and. mod(step-ini_particle_time,parti_inifreq) .eq. 0d0) then
      CALL ini_particles(step)
    endif      
