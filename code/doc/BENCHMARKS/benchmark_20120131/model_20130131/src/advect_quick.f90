...
!     periodic-ew boundaries  : do i=1,NI  (instead of i=1,NI-1)        
            ...
            s(0,j,k,m)= s(NI,j,k,m) 
            T(0,j,k,m)= T(NI,j,k,m) 
            u(0,j,k,m)= u(NI,j,k,m) 
            v(0,j,k,m)= v(NI,j,k,m) 
            w(0,j,k,m)= w(NI,j,k,m) 
     ...
     &                 w(i+1,j,k,m),dwx(i),dwx(i-1),courinv,fc)         
                  ...
                  do it=1,ntr 
                     right= Eighth*(dTrx(it,i2+1) -dTrx(it,i)) 
                     ctr= Half* (Tr(it,i2+1,j,k,m) +Tr(it,i,j,k,m)) 
                     fc= ctr - right 
                     call ultim(Tr(it,i2+2,j,k,m),Tr(it,i2+1,j,k,m),      &
     &                    Tr(it,i,j,k,m),dTrx(it,i2),dTrx(it,i2+1),       &
     &                    courinv,fc)                                   
                     Trflx(it,i,j,k)= uf(i,j,k)*fc 
                  end do 
                                                                        
                  right= Eighth*(dsx(i2+1) -dsx(i2)) 
                  ctr= Half* (s(i2+1,j,k,m) +s(i,j,k,m)) 
                  fc= ctr - right 
                  call ultim(s(i2+2,j,k,m),s(i2+1,j,k,m),                 &
     &                 s(i,j,k,m),dsx(i),dsx(i2+1),courinv,fc)           
                  sflx(i,j,k)= uf(i,j,k)*fc 

                  right= Eighth*(dTx(i2+1) -dTx(i)) 
                  ctr= Half* (T(i2+1,j,k,m) +T(i,j,k,m)) 
                  fc= ctr - right 
                  call ultim(T(i2+2,j,k,m),T(i2+1,j,k,m),                 &
     &                 T(i,j,k,m),dTx(i),dTx(i2+1),courinv,fc)           
                  Tflx(i,j,k)= uf(i,j,k)*fc 
                                                                        
                  right= Eighth*(dux(i2+1) -dux(i)) 
                  ctr= Half* (u(i2+1,j,k,m) +u(i,j,k,m)) 
                  fc= ctr - right 
                  call ultim(u(i2+2,j,k,m),u(i2+1,j,k,m),                 &
     &                 u(i,j,k,m),dux(i),dux(i2+1),courinv,fc)           
                  uflx(i,j,k)= uf(i,j,k)*fc 
                                                                        
                  right= Eighth*(dvx(i2+1) -dvx(i)) 
                  ctr= Half* (v(i2+1,j,k,m) +v(i,j,k,m)) 
                  fc= ctr - right 
                  call ultim(v(i2+2,j,k,m),v(i2+1,j,k,m),                 &
     &                 v(i,j,k,m),dvx(i),dvx(i2+1),courinv,fc)           
                  vflx(i,j,k)= uf(i,j,k)*fc 
                                                                        
                  right= Eighth*(dwx(i2+1) -dwx(i)) 
                  ctr= Half* (w(i2+1,j,k,m) +w(i,j,k,m)) 
                  fc= ctr - right 
                  call ultim(w(i2+2,j,k,m),w(i2+1,j,k,m),                 &
     &                 w(i,j,k,m),dwx(i),dwx(i2+1),courinv,fc)           
                  wflx(i,j,k)= uf(i,j,k)*fc 
               end if 
