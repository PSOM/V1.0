...
#include "cppdefs.h"
 ...
 DO j=0,NJ/2
   Fy=tanh((dble(j)-dble(15))/dble(30)*2.d0*PI)
   dyM(j)=abs(Fy-1.d0)/2.d0*dy*3.d0+dy
 ENDDO
 DO j=NJ/2+1,NJ+1
   Fy=tanh((dble(j)-dble(NJ-15))/dble(30)*2.d0*PI)
   dyM(j)=abs(Fy+1.d0)/2.d0*dy*3.d0+dy
 ENDDO
...
!do k=0,NK+1; print*,k,zc(NI/2,10,k); end do;
 ...
 ! Writes the horizontal grid into a file
   OPEN (unit=61,file=TRIM(dirout)//'horizontalgrid.out')                                              
   WRITE(61,*) 'x-axis 0-based index, distance (m)'
   do i=0,NI+1; WRITE(61,*) i,xc(i)*1000.; end do;
   WRITE(61,*) 'y-axis 0-based index, distance (m)'
   do j=0,NJ+1;WRITE(61,*) j,yc(j)*1000.; end do;
   CLOSE(61)
   
   ! Writes the timestep in seconds from t=0
   OPEN (unit=62,file=TRIM(dirout)//'timegrid.out')                                                
   WRITE(62,*) 'timestep, time in seconds' 
   do i=0,nsteps; WRITE(62,*) i,i*dtime_dim; end do;
   CLOSE(62) 
