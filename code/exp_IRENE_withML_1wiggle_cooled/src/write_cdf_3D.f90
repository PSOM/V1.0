...
!----------------------------------------------------------             
  ...
  USE header, only : NI,NJ,NK,xc,yc,zc,p,h,consump,T,s,rho,Tr,u,v,w,vor,pv,uf,vf,wf,Kz,conv,&
      & con100,nconsume,dirout,rc_kind,ntr,swr,stress_top_x,stress_top_y,DL,UL,WL,EPS,Jac
#include "netcdf.inc"                                                                        
  ...
  integer :: idvx,idvcon100,idFaceFile,idvuf, idvvf, idvwf, idvKf, idvjac
  ...
  ! Output file names                                              
  WRITE(outname_data,'("Jac.cdf")')    ! Cell centers 
  WRITE(outname_face,'("face_",I6.6,".cdf")') stepl     ! Cell faces
  ...
  dimuf(1) = ncddef(idDatFile,'xf_x',NI+1,rcode)
  dimuf(2) = ncddef(idDatFile,'yf_x',NJ,rcode)
  dimuf(3) = ncddef(idDatFile,'zf_x',NK,rcode)

  dimvf(1) = ncddef(idDatFile,'xf_y',NI,rcode)
  dimvf(2) = ncddef(idDatFile,'yf_y',NJ+1,rcode)
  dimvf(3) = ncddef(idDatFile,'zf_y',NK,rcode)

  dimwf(1) = ncddef(idDatFile,'xf_z',NI,rcode)
  dimwf(2) = ncddef(idDatFile,'yf_z',NJ,rcode)
  dimwf(3) = ncddef(idDatFile,'zf_z',NK+1,rcode)
  ...
  idvh = ncvdef(idDatFile,'J2d',NCDOUBLE,2,dims2d,rcode)
  idvs = ncvdef(idDatFile,'Jac',NCDOUBLE,3,dims,rcode)
...
!  CALL NCAPTC (idDatFile, idvmld,'units', NCCHAR, 1,'m', RCODE)  
  ...
  CALL ncvpt(idDatFile,idvs, start, count, Jac, rcode)
...
!  CALL ncvpt(idDatFile,idvHF, start(2), count(2), swr, rcode)
  CALL ncvpt(idDatFile,idvh, start2d, count2d, J2d, rcode)
...
!  idvKf = ncvdef(idFaceFile,'Kz',NCDOUBLE,3,dimwf,rcode)
  ...
  CALL ncvpt(idFaceFile,idvuf, start, countuf, uf*UL, rcode)
  CALL ncvpt(idFaceFile,idvvf, start, countvf, vf*UL, rcode)
  CALL ncvpt(idFaceFile,idvwf, start, countwf, wf*WL/EPS, rcode)
