...
! end do 
 ...
 if(verbose) then
   ...
   write(6,"(A,3(F8.2),A2)")" # Grid size dx, dy, dz_avg            : ",dx,dy,dz_avg, " m"
#ifdef fixed_bottom_thickness
   write(6,"(A,2(F8.2),A2)")" # Top & bottom layer thickness        : ",dztop*DL,dzbot*DL, " m" 
#else
   write(6,"(A,F8.2,A)")    " # Top & bottom layer thickness        : ",dztop*DL, "variable m" 
#endif
   ...
   write(6,"(A,D10.3,A6)") " # Background stratification: ",Nbkgrnd," rad/s"

   if(fnhhy .ge. 0.9) then
     write(6,*) "# Hydrostatic approximation DISABLED"
   else
     write(6,*) "# Hydrostatic approximation ENABLED"
   endif
...
!      endif
   ...
   ! =====================================================================================
   write(6,*) "                                                                          "
   write(6,*) "---------------------  DISSIPATION CHARACTERISTICS  ----------------------"
   write(6,*) "                                                                          "
      
   if (iv_compute_kz==0) then
         write(6,*) "# Parameterization of horizontal/vertical viscosities DISABLED/DISABLED"
   else
      if (iv_compute_kh==1) then
         write(6,*) "# Parameterization of horizontal/vertical viscosities ENABLED/ENABLED"
      else
         write(6,*) "# Parameterization of horizontal/vertical viscosities DISABLED/ENABLED"
      endif
      write(6,"(A,   F5.2 )") " # Critical gradient Froude number       : ",gFrc
   endif 
   
   if (biharmonic_horizontal) then
      write(6,"(A,2(D10.3),A)") " # Horizontal mixing coefficient (m4.s-1): ",B0x,B0y," (biharmonic)"
   else
      write(6,"(A,2(D10.3),A)") " # Horizontal mixing coefficient (m2.s-1): ",A0x,A0y," (Laplacian)"
   endif
      write(6,"(A,2(D10.3),A)") " # Vertical mixing coefficient (m2.s-1)  : ",KzMom,KzTr," (Laplacian)"
