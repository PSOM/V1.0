...
!       enddo
      ...
      elseif (selvar==3 .OR. selvar==4 .OR. selvar==5) then
...
!    integer, parameter :: n1=3
   REAL(kind=rc_kind), parameter :: r=0.2d0
   REAL(kind=rc_kind) :: grho,K1 !,Kz_max,Kz_min,gFr_max,gFr_min !, Ri
   REAL(kind=rc_kind), dimension(0:NI+1,0:NJ+1,0:NK+1) :: dudz,dvdz,drdz,vshear,bvfreq,gFr
...
!  Kz is not needed at k=0 and NK since stress bcs are used.         
   Kz(:,:, 0)= 0.d0  ! size of Kz is (0:NI+1,0:NJ+1,0:NK+1)
   Kz(:,:,NK)= 0.d0 
...
!             endif 
         ...
         if (gFr(i,j,k).gt.gFrc) then
            Kz(i,j,k)=K1+ r*dz_avg**2*Nbkgrnd/96 *(gFr(i,j,k)**2-gFrc**2)*(gFr(i,j,k)-gFrc)
         else
            Kz(i,j,k)=K1
         endif
...
!    gFr_max = maxval(gFr(1:NI,1:NJ,1:NK-1))
!    gFr_min = minval(gFr(1:NI,1:NJ,1:NK-1))
!    Kz_max = maxval(Kz(1:NI,1:NJ,1:NK-1))
!    Kz_min = minval(Kz(1:NI,1:NJ,1:NK-1))
! 
!    write(6,"(A,2(D15.7))") " # K1: " ,K1
!    write(6,"(A,2(D15.7))") " # gFr is between: " ,gFr_max,gFr_min
!    write(6,"(A,2(D15.7))") " # Kz is between: " ,Kz_max,Kz_min
