subroutine psolve(p,divQ)
  ...
  REAL(kind=rc_kind) :: cp(19,NI,NJ,NK),rhs(NI,NJ,NK),                   &
       ...
       &     dtime,tol,maxres,pstar,rlx,edt,EPS
  !
 ...
 !-----------------MGFILL PART------------------
     ...
     &              - rhs(i,j,k) )/(-cp(1,i,j,k))
            ...
            p(i,j,k)= (1.d0 -rlx)*p(i,j,k) +rlx*pstar
               do 130 i=2,NI-1
                 !print*, 'psolve.f90: cp=', cp(1,i,j,k),cp(2,i,j,k),cp(3,i,j,k),cp(4,i,j,k),cp(5,i,j,k),cp(6,i,j,k),cp(7,i,j,k),cp(8,i,j,k),cp(2,i,j,k),cp(2,i,j,k)
