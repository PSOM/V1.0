subroutine write_cdf_2D_x(islice,counter_2d,n)
  ...
  !     ------------------------------------------------------------      
  USE header,ONLY : NI,NJ,NK,ntr,s,T,rho,u,v,w,consump,Tr,shear,vor,freqN2,yc,zc,fricb,stress_top_x,DL,time_seconds,dirout,rc_kind, pvt, rp, stress_top_y, pv3, KzMom, KzTr
...
!     Zonally averaged psi and by,bz                                    
 ...
 integer :: iddatfile, idigit, idudx, idudy, idudz, idvbysection, idvby, idvbz, idvcon, idvcy, idvbx
  ...
  integer :: idvzave,idvshear, idvpvt, idvpv3, idvkzm, idvkzt
...
!            shearslice(j,k)=shear(i,j,k)
            ...
            kzmsli (j,k)= KzMom(i,j,k) 
            kztsli (j,k)= KzTr (i,j,k) 
            pslice (j,k)= rp   (i,j,k) 
!            do itr=1,ntr 
...
!     write to a netcdf file                  
      ...
      if (counter_2d.eq.1) then 
         ...
         idvstressx= ncvdef(idDatFile,'stressx',NCFLOAT,2,dimswind,rcode) 
         idvstressy= ncvdef(idDatFile,'stressy',NCFLOAT,2,dimswind,rcode) 
        ...
        ! idvtbar = ncvdef(idDatFile,'trbar',NCFLOAT,4,dims4,rcode) 
         ...
         idvkzm = ncvdef(idDatFile,'kzm',NCFLOAT,3,dims,rcode) 
         idvkzt = ncvdef(idDatFile,'kzt',NCFLOAT,3,dims,rcode) 
...
!         call ncredf(idDatFile,rcode)                                  
         ...
         idvkzm = NCVID(idDatFile, 'kzm', RCODE) 
         idvkzt = NCVID(idDatFile, 'kzt', RCODE) 
...
!         idvpec = NCVID(idDatFile,'psieddytracer',rcode)               
      ...
      CALL ncvpt(idDatFile,idvkzm, start, count,  kzmsli, rcode) 
      CALL ncvpt(idDatFile,idvkzt, start, count,  kztsli, rcode) 
